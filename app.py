...
if "queue" not in st.session_state:
    st.session_state.queue = []          # lista de task_ids na ordem
if "tasks" not in st.session_state:
    st.session_state.tasks = {}          # task_id -> dados do job
...
if st.session_state.selected_country is None:
    ...
    country = st.selectbox("Voice / Language", options=list(COUNTRY_VOICES.keys()))
    ...
    with st.sidebar:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                st.metric("Remaining", f"{max(0, limit - used):,}")
                st.progress(used / limit if limit else 0)
        except:
            pass

        st.divider()
        force_fit = st.checkbox("Force fit timing (truncate if needed)", value=False)
    ...
    if uploaded_file:
        try:
            ...
            st.info(f"**{len(valid)} segments** • **{chars:,} characters** • Est. cost: **${chars/1000*0.10:.2f}**")
            os.remove(tmp)
        except:
            pass
    ...
    with col1: stability = st.slider("Stability", 0.0, 1.0, 0.60, 0.05)
    with col2: similarity = st.slider("Similarity Boost", 0.0, 1.0, 0.85, 0.05)
        ...
        if task.get("status") == "queued":
            ...
            with st.container(border=True):
                ...
                if task.get("truncated"):
                    st.markdown("**Truncated:** " + ", ".join([f"{n} (cut {m}ms)" for n, m in task["truncated"]]))
                if task.get("sped_up"):
                    st.markdown("**Sped up:** " + ", ".join([f"{n} ({s:.2f}x)" for n, s in task["sped_up"]]))
...
st.divider()
st.caption("Queue version • One file at a time • Made for content creators")
